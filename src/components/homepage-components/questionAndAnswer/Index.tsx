    ...
    <div className="max-w-[90%] lg:max-w-[80%] mx-auto py-12">
      <div className='grid grid-cols-1 lg:grid-cols-2 gap-5'>
        <div className="w-[80%]">
          ...
          <AnimatedText text="Most questions that are asked by people." />
          ...
          <div className="w-full">
            {faqItems.map((item, index) => (
              <div key={index} className="mb-3 rounded-lg bg-white shadow-md">
                <div
                  className="flex cursor-pointer items-center justify-between w-full p-4 focus:outline-none"
                  onClick={() => toggleAccordion(index)}
                >
                  <span className={`text-md lg:text-lg font-semibold ${openIndex === index ? 'text-purple-600' : 'text-black'}`}>
                    {item.title}
                  </span>
                  <span className={`text-5xl ${openIndex === index ? 'text-purple-600' : 'text-black'}`}>
                    {openIndex === index ? '-' : '+'}
                  </span>
                </div>
                {openIndex === index && (
                  <p className="pb-4 px-4">{item.content}</p>
                )}
              </div>
            ))}
          </div>
